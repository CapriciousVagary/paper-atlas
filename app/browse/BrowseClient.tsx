...
import { applyPaperOverrides, authorRoleLabels, categories, getAuthorDetails, getClassifications, hasClassification, paperAddedAt, papers, type Paper } from "../data";
...
export default function BrowseClient({ initialQuery }: { initialQuery: Query }) {
  ...
  const filtered = subcategoryPapers.filter((paper) => {
    ...
    if (author) {
      const matches = getAuthorDetails(paper).some((item) => (item.name === author || item.aliases?.includes(author)) && (!institution || item.institution === institution));
  ...
  return <div className="browse-shell">
    ...
    <div className="browse-layout">
      <aside className="filter-panel">
        <section><b>大类</b><Link className={!category ? "active" : ""} href={href({ category: "", subcategory: "", tag: "", page: "1" })}>全部方向 <span>{allPapers.length}</span></Link>{categoryNames.map((name) => <Link className={category === name ? "active" : ""} href={href({ category: name, subcategory: "", tag: "", page: "1" })} key={name}>{name}<span>{allPapers.filter((paper) => hasClassification(paper, name)).length}</span></Link>)}</section>
