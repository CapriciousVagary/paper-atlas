...
export async function GET(request: Request) {
  ...
  const url = new URL(request.url); const slug = url.searchParams.get("slug")?.trim() ?? ""; const index = Math.max(0, Number.parseInt(url.searchParams.get("index") ?? "0", 10) || 0);
  let keys: string[] = [];
  const [paper] = await getDb().select({ figureKeys: papers.figureKeys }).from(papers).where(eq(papers.slug, slug)).limit(1);
  if (paper) keys = parseJson(paper.figureKeys, []); else { const [edit] = await getDb().select().from(paperEdits).where(eq(paperEdits.slug, slug)).limit(1); keys = parseJson<{ _figureKeys?: string[] }>(edit?.data, {})._figureKeys ?? []; }
  const key = keys[index]; if (!key) return new Response("Not found", { status: 404 });
  const object = await (env as unknown as { PAPERS: R2Bucket }).PAPERS.get(key); if (!object) return new Response("Not found", { status: 404 });
...
export async function POST(request: Request) {
  if (!isAuthorized(request)) return Response.json({ error: "审批码无效" }, { status: 401 });
  await ensureDatabase();
  const payload = await request.json() as { slug?: string; recordType?: "curated" | "stored"; figureKeys?: string[]; figureCaptions?: string[]; keyFigureIndex?: number; clear?: boolean };
  const slug = String(payload.slug ?? "").trim();
  const recordType = payload.recordType;
  if (!slug || !recordType) return Response.json({ error: "缺少论文或条目类型" }, { status: 400 });
  const prefix = `papers/${slug}/figures/`;
  ...
  const figureCaptions = nextKeys.map((_, index) => String(payload.figureCaptions?.[index] ?? "").trim().slice(0, 1000));
  const caption = figureCaptions[keyIndex] ?? "";
  ...
  if (recordType === "stored") {
    const [paper] = await getDb().select({ figureKeys: papers.figureKeys, figureCaptions: papers.figureCaptions, figureCaption: papers.figureCaption }).from(papers).where(eq(papers.slug, slug)).limit(1);
    ...
    await getDb().update(papers).set({ figureKeys: JSON.stringify(nextKeys), figureCaptions: JSON.stringify(figureCaptions), keyFigureKey, figureCaption: caption }).where(eq(papers.slug, slug));
  } else {
    ...
    const data = { ...existing, _figureKeys: nextKeys, _keyFigureKey: keyFigureKey, figureImageUrl: keyFigureKey ? `/api/papers/${encodeURIComponent(slug)}/figure` : "", figureImageUrls: nextKeys.map((_, index) => `/api/papers/${encodeURIComponent(slug)}/figure?index=${index}`), figureCaptions, figureCaption: caption };
