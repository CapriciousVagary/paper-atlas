...
export async function POST(request: Request) {
  ...
  const payload = await request.json() as { slug?: string; pdfKey?: string | null; figureKeys?: string[]; figureCaptions?: string[]; keyFigureIndex?: number };
  ...
  const figureCaptions = figureKeys.map((_, index) => String(payload.figureCaptions?.[index] ?? "").trim().slice(0, 1000));
  const keyIndex = Math.min(Math.max(Number(payload.keyFigureIndex) || 0, 0), Math.max(figureKeys.length - 1, 0));
  await getDb().update(papers).set({ fileKey: pdfKey, figureKeys: JSON.stringify(figureKeys), figureCaptions: JSON.stringify(figureCaptions), keyFigureKey: figureKeys[keyIndex] ?? null, figureCaption: figureCaptions[keyIndex] ?? "" }).where(eq(papers.slug, slug));
