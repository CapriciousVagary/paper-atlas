...
type UploadedPaper = Pick<Paper, "slug" | "title" | "category" | "subcategory" | "journal" | "published"> & {
  authors?: string[];
  institutions?: string[];
};
    ...
    <main>
      ...
      <section className="hero-shell">
        <div className="hero-copy">
          <span className="eyebrow">Q-CHIP SHARED RESEARCH MEMORY</span>
          <h1>把读过的论文，变成<br /><em>可检索的集体知识。</em></h1>
          <p>围绕光计算、薄膜铌酸锂与集成激光器，沉淀中文摘要、关键图表和一眼可回忆的创新点。</p>
          ...
          <div className="hero-stats">
            ...
            <span><strong>{categories.reduce((sum, item) => sum + item.subcategories.length, 0)}</strong> 个专题</span>
            <span><strong>3</strong> 个主方向</span>
          </div>
        </div>
        <div className="knowledge-map" aria-label="研究方向知识图谱">
          <div className="map-grid" />
          <div className="map-center"><span>RIQT</span><strong>Q-chip</strong></div>
        ...
        <div className="paper-list">
          {visiblePapers.length ? visiblePapers.map((paper, index) => (
            <article className="paper-card" key={paper.slug}>
              ...
              {paper.figureType ? <div className="paper-visual"><FigurePreview type={paper.figureType} /><Link href={`/papers/${paper.slug}`} aria-label={`查看 ${paper.title}`}>阅读笔记 ↗</Link></div> : <Link className="paper-open" href={`/papers/${paper.slug}`}>打开条目 →</Link>}
            </article>
      ...
      <section className="contribute-banner">
        <div><span className="section-kicker">CONTRIBUTE TO THE ATLAS</span><h2>读完一篇，留下一个可复用的入口。</h2><p>上传 PDF 或填写 DOI，按模板补全信息；后续可接入自动解析生成初稿。</p></div>
        <Link href="/upload">上传并创建条目 <span>↗</span></Link>
      </section>
      <footer><span>RIQT · Q-chip Literature Atlas</span><span>Research memory, shared.</span></footer>
    </main>
  );
}
